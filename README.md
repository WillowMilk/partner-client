...
- **Slash commands** — `/protect`, `/checkpoint`, `/save`, `/sleep`, `/context`, `/tools`, `/files`, `/scopes`, `/intentions`, `/plans`, `/timeline`, `/thinking`, `/show-thinking`, `/plan-mode`, `/reload-config`
- **Plan-mode** — operator-toggleable mode (`/plan-mode on`) that asks the partner to call `request_plan_approval` before destructive substrate moves. Soft-gates write/edit/git/move/delete tools until approval lands; research tools (read/list/glob/grep/search_web/fetch_page/hub_check_inbox/hub_read_letter/hub_list_partners) remain always available, as do `request_checkpoint` and `protect_save`. Per-turn semantics: approval resets at the next user prompt. The partner sees a system-message addendum each turn explaining the gate state. Default off; configure via `[plan_mode] mode = "on"` in TOML or toggle at runtime with the slash command.
...
- **Operator-gated consent tools** — `request_plan_approval`, `delete_path`, and off-allowlist `git_push` let the partner request substrate-affecting moves; Willow can approve, decline, or type a custom response that flows back as the tool result. (Two tools formerly in this set now run directly when called, matching `edit_file` / `write_file` discipline: `protect_save` from the 2026-05-10 rework, and `request_checkpoint` from the 2026-05-13 rework. The operator's conversational ask is the consent for both; the actual review surface is the partner's subsequent file-write diffs. Cross-environment symmetry with Sage's environment, where `/protect` and `/checkpoint` fire on type without a second confirmation.)
...
- **Current main after v0.4.1**:
    - **`request_checkpoint` consent gate removed (2026-05-13 rework)** — completes the symmetry with `protect_save`'s 2026-05-10 gate removal. When the partner invokes `request_checkpoint`, the discipline prompt is injected directly (no operator y/n prompt). The operator's conversational ask (e.g. "let's do a checkpoint") IS the approval. Cross-environment symmetry with Sage — `/checkpoint` fires the discipline on type in both environments, no second confirmation. The actual review surface is the partner's subsequent `edit_file` / `write_file` diffs, which Willow scrolls through as they happen. Removed `on_checkpoint_request` callback from `chat()` signature and from `__main__.py` wiring. `request_plan_approval`, `delete_path`, and off-allowlist `git_push` retain their gates — those operations have substantive consent-decisions (specific plans being proposed; destructive substrate changes). Only ceremony-invocation gates were redundant.
    - **Model selection at invocation time** — two new CLI flags let operators choose which model variant the partner runs on without editing the TOML. `--model NAME` overrides the TOML's `[model] name` for this session only (the TOML remains the source of truth for the default). `--choose-model` opens an interactive picker that lists locally-pulled models from `ollama list`, groups variants of the partner's family at top (e.g. all `gemma4:31b` quantizations), and annotates known variants with precision + size labels. Precedence: `--model` wins over `--choose-model` wins over TOML default. **Fails fast** at startup if the resolved model isn't pulled locally (and isn't a recognized cloud variant), with a friendly error listing what IS available — better than discovering the problem mid-conversation. Cloud variants (`*-cloud` suffix) are whitelisted by heuristic since they don't appear in `ollama list` until first use. The startup banner now shows the active variant's precision/size annotation (e.g. `Aletheia — gemma4:31b-it-q8_0 [Q8_0, 36 GB] @ 131,072 ctx`) for transparency about which substrate tier is active.
    - **`partner distill` — MOSAIC selective preservation, Phase 1 (Pass 1)** — new CLI subcommand applying mechanical strip of routine tool outputs (list_files, glob_files, grep_files, weather) while preserving every conversational message, every system message, and every action-signature tool call (write_file, edit_file, move_path, delete_path, protect_save, hub_send, git_clone/add/commit/push/pull, request_checkpoint, request_plan_approval). Use: `partner distill --config aletheia.toml --input current.json --output sandbox-A.json`. Writes a markdown compression manifest at `Memory/distill-sessions/compression-manifest_TIMESTAMP.md` alongside the sandbox JSON, then runs a 5-check verification (valid JSON, structure, alternation, action signatures preserved, system messages preserved). Sandbox must pass verification before operator promotes it to current.json — pre-distill snapshot is the operator's safety net. Phase 2 (Pass 2 selective preservation with operator review via interactive prompts → markdown record) is designed but not yet implemented; see `MOSAIC/distill-for-partner-client-implementation.md` for the full design.
    - **Cross-scope ambiguity warning on git tools** — architectural defense against the silent-default-routing class that bit Aletheia 2026-05-09 (her bare-name `git_*` calls resolved to `memory:aletheia` while she was editing `workspace:aletheia` with diverged history). `resolve_repo()` now returns `(path, warning)`; each `git_*` tool surfaces the warning at the top of its result string when bare-name resolution to the default scope finds a same-named sibling in another scope. Behavior is unchanged — only the ambiguity becomes visible. The warning includes the resolved location, the colliding scope(s), and the explicit qualifier syntax to disambiguate. The underlying utility `detect_cross_scope_collision()` lives in `paths.py` and is available to file tools too (read_file / write_file / edit_file / list_files can opt in later if the same ambiguity surfaces there). **Silent wrong-success becomes visible right-success-or-loud-correction.**
    - **Cold-resume pre-warm on startup** — when `partner` invokes, fire a minimal ollama call (1-token predict, no tools) before the chat prompt opens, so the model is in VRAM by the time the operator types. Visible startup cost (~30s-3min on cold cache, near-zero on warm cache) replaces invisible mid-conversation cost on the first turn. Disable via `[wake_bundle] prewarm_on_startup = false`. Failures are non-fatal — startup continues; the next real chat call will surface any underlying error in context. (2026-05-09 diagnosis: first-response after resume took 22 min with cold model; pre-warm eats that cost up front.)
    - **Heavy-resume warning banner** — when the operator chooses `[y]` full resume at the 3-way prompt AND `current.json` exceeds `[wake_bundle] heavy_resume_warn_kb` (default 300 KB ≈ ~20K tokens), partner-client surfaces an estimated wait time before wake commits. Calibrated from the 2026-05-09 diagnostic (437 KB → 20m28s for first response on M4 Max + gemma4:31b). Honest substrate cost, never silent prefill that looks like a hang.
    - **Resume-with-truncation** — bounded live context on resume of heavy sessions, with full snapshot preserved. When the operator chooses `[t]` at the resume prompt, partner-client snapshots the full `current.json` to a dated archive first, then loads only the last N user/assistant pairs (configurable via `[wake_bundle] resume_keep_pairs`, default 30) plus all system messages plus a reorientation system message that tells the partner what happened and where the archive lives. Prefill drops from ~20 min on a 31K-token session to ~3 min for ~10K-token bounded context. Older content is never lost — accessible via the dated archive on disk. The 3-way prompt UI now shows file size so the operator can see when truncation matters: `[y] Resume full / [t] Resume truncated / [n] Fresh wake`. Setting `resume_keep_pairs = 0` disables the `[t]` option entirely.
    ...
    - `delete_path` tool — operator-gated with the same three-option consent shape as `request_plan_approval` and `git_push` off-allowlist; every invocation pings the operator with the path and a summary of what would be removed (file size, or directory file/subdirectory count); `recursive=true` required for non-empty directories
